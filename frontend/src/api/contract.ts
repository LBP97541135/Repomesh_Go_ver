...
export interface IssueListItemView {
  ...
  /** Go as-built 详情/列表自带的来源引用：主会话 id 是对话流主体的定位键
   *  （消费方 rooms.ts / WorkbenchPage）。Python 读模型没有这一栏，按可选收。 */
  source?: { kind: string; conversationId: string } | null;
}
...
export interface RoomListItemView {
   ...
   *  文案不得写「在线」。 */
  live: boolean;
  /** Go as-built(issues rooms 读面)附带的会话 id——消息流改走
   *  conversations messages 端点时用它定位。两种命名都收,防御性可选。 */

...
export function fetchProjectTopology(projectId: string): Promise<ProjectAgentTopologyView> {
  /** Go 路由是 /api/projects/{projectId}/topology(无 v1 段)——旧 sessionRequest
   *  的 /api/v1 前缀在这里是 404 错位,改走 apiRequest(as-built,2026-09-17 对齐)。 */
  return apiRequest<ProjectAgentTopologyView>(
    "GET",

...
const names = {overview:'概览',traces:'Trace 与事件',trials:'评测与验收',datasets:'数据集',settings:'设置'};
...
async function refresh() {
  try {const [c,s]=await Promise.all([api('/api/catalog'),api('/api/settings')]);catalog=c;settings=s;render();$('#updated').textContent=`更新于 ${new Date(c.updated_at).toLocaleTimeString('zh-CN')}`;}
...
function render() {
  ...
  let html='';
  if(view==='overview') {
    html=`<section class="hero"><div><h2>观测、证据和验收，都在本机</h2><p>从检查结果追溯到实际请求、固定版本与输入证据。可选 DeepSeek 评审单独记录，业务验收保留原始结论。</p></div>${btn('run','suite','','运行本地验收','primary')}</section>
    ...
    html=`<section class="hero"><div><h2>本地模式已就绪</h2><p>数据存储和确定性评测不依赖云服务。无需配置 AgentSpace、地域或阿里云凭据。</p></div><span class="badge pass">本地运行</span></section><section class="panel"><h2>观测评估模型 · DeepSeek</h2><p>可选。点击某条验收的「AI 契约评审」时，才会把该条验收报告发送给 DeepSeek；结果仍保存在本机。</p><form id="model-form"><label class="field"><span>观测评估模型名称</span><input name="model" value="${esc(settings.model||'deepseek-flash')}" required autocomplete="off"></label><label class="field"><span>观测评估 API Key ${settings.model_configured?'· 已保存，留空保留':''}</span><input type="password" name="api_key" autocomplete="new-password" placeholder="${settings.model_configured?'已安全保存':'填写 DeepSeek API Key'}"></label><div class="row"><button type="submit" class="primary">保存模型配置</button>${btn('testModel','','','测试连接','secondary')}<small>${settings.model_configured?'凭据已配置，页面不会回读密钥':'尚未配置'}</small></div></form><div class="divider"></div><dl class="kv"><dt>观测评估 API</dt><dd><code>${esc(settings.model_endpoint)}</code></dd><dt>评审规则</dt><dd>contract-handoff/1 · 有证据才给出确定结论</dd><dt>自动上传</dt><dd>关闭。规则验收不会调用模型。</dd></dl></section><section class="panel"><h2>本地数据与接入</h2><dl class="kv"><dt>OTLP HTTP</dt><dd><code>${esc(location.origin)}/v1/traces</code><div class="help">支持 protobuf / JSON，使用实际监听端口配置本机采集器。</div></dd><dt>页面资源</dt><dd>随程序提供，不使用外部 CDN</dd><dt>运行链路</dt><dd>DSH 尚未接入；模型成本仅展示真实评审响应中的用量</dd></dl>${(settings.archives||[]).map(a=>`<div class="source"><code>${esc(a.path)}</code><span class="badge">${a.writable?'读写':'只读'}</span></div>`).join('')}</section>`;
  }
...
async function perform(button) {
  ...
  try {
    ...
    if(action==='run'||action==='runSelected') {
      ...
      notice('正在检查 DeepSeek 凭据和模型列表…');const result=await api('/api/model/test',{});
      notice(result.ok?`连接成功，${settings.model} 可用。`:`凭据有效，但所选模型不在列表中。可用模型：${result.models.join('、')}`,!result.ok);
    } else if(action==='judge') {
      notice('正在调用 DeepSeek 评审所选报告…');const result=await api('/api/judge',{archive,trial_id:id});
  ...
  } catch(e){notice(e.message,true);} finally{busy=false;button.disabled=false;document.querySelectorAll('button:disabled').forEach(b=>b.disabled=false);}
}
...
document.addEventListener('submit',async e=>{
  ...
  try{await api('/api/model',{model:values.get('model'),api_key:values.get('api_key')},'PUT');key.value='';notice('DeepSeek 配置已保存到本机私有文件。');await refresh();}
  catch(err){notice(err.message,true);}finally{key.value='';busy=false;document.querySelectorAll('button:disabled').forEach(b=>b.disabled=false);}
});

...
2026-09-19 采用补充：§6 的项目创建仓库数组改为允许空数组，替代原非空要求；§7 明确增仓及原操作重放语义保留。新控制台按“保存项目 → 接入仓库 → 创建 Issue”操作，项目详情的配置摘要不能代替 Issue 创建条件。实现和证据见[范围修复记录](../development/2026-09-19-project-scope/README.md)。
...
name 必填、非空白、最多200；purpose 必填、非空白、最多20000；repositoryIds 必须提供数组、去重，允许 `[]`，最多100个稳定身份（2026-09-19 顺序修正：先保存项目，再明确接入仓库；空项目不可创建 Issue）。首批项目总范围上限100，作为本轮可调整的明确规模参数。configuration 可省略，等价两个 inherit；提供时必须包含完整两项引用。用户／owner、权限、分支、startRuntime 等字段一律不接受。

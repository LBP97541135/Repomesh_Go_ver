...
独立观测工作台默认页改为任务地图：按一次 Trial 或 OTLP Trace 组织任务故事线、性能泳道和证据链，再下钻到现有完整详情；原九个数据页和档案结构保持不变。Trial 缺少共享运行时 Trace 时不拼造时间线，Trace 仅按真实 parent／起止时间排列；聚合页不覆盖业务验收、Jev 判断或人工标签。

目标包测试、构建／vet、race 和浏览器验证通过。浏览器覆盖固定 Trial 与两层 OTLP 协议夹具、完整 Span 属性／事件／Links、移动端、九个原页面及零外部请求；协议夹具不等于真实 Agent／DSH 验收。范围和证据见[任务地图实施记录](../development/2026-09-20-observation-task-map/README.md)。原生 DSH、真实流式链路和真实任务效果仍按既有边界待验。

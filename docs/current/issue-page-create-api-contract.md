...
- 2026-09-19 实施补充：可选仓库必须属于当前项目且实时用户读取和 App 工作能力允许。`canSubmit` 同时要求固定配置具备模型绑定、完整执行配方和可用的运行时 App 密钥；配置缺失、无可选仓库、App 密钥未核实分别返回 `CONFIGURATION_NOT_READY`（无固定配置时沿用 `MODEL_CONFIG_MISSING`）、`NO_AVAILABLE_REPOSITORIES`、`APP_AUTHORIZATION_UNCONFIRMED`。它基于完整项目范围计算，与本页是否恰有可选项无关。客户端续读 `nextCursor` 并核对同一 `creationContextRevision`；创建时固定项目、明确仓库子集和修订，原次重试沿用确切输入与键。

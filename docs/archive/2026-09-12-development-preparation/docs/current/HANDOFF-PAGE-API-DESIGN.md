...
**当前执行范围：页面与API设计，开发之后单独做。** 用户已确认[首批六包待做表](first-development-todo.md)，当前优先展示[最小Issue详情概览r2](../../../../current/issue-overview-minimal-design.md)，等待三项UI裁决；后端按表准备认证／配置来源待审草案。新决定仍逐项同意，开发未启动。下方“先交表／尚未确认范围”为此前状态。

**当前任务已收拢：先交开发待做表。** 用户同意F04三项UI后要求只做最关键基本页面，见[首批待做表建议](first-development-todo.md)。现在暂停顺序扩展F05—F15原型，表中具体首批范围仍待用户确认；不自动开工。F04三项UI已采用，专用协议未完成，F02技术0／3保持。

此前F04展示： 用户“同意，继续下一项”采用F03三项页面安排，转入[模型应用预览](../../../../current/model-project-apply-design.md)；当时未采用F04，现以顶部最新用户决定为准。F03技术r1仍1／3、F02技术仍0／3，所有整行未完保持。

此前F03展示： 用户“继续下一项”，先在已采用F02的添加仓库区域展示选择摘要、搜索／加载更多、授权观察及回表单保存。见[本页提案](../../../../current/repository-picker-design.md)；现已获三项UI采用，技术收发见页面CP09／CP10，F02及其余未完项保留。

**最新用户顺序：先阅读并串联整个原型目录，再讨论。** 当前已准备[整体串联r1](../../../../prototypes/index.html)及[来源与路径说明](../../../../current/prototype-walkthrough-design.md)，保留15份原稿和已有采用范围。新增整体导航／跨页衔接待用户评审，F02三项页面安排已获同意，并要求主界面可找到（见专题与页面CP08），F01—F15未完成状态不变。

**当前协作与决策规则（2026-09-11用户追加）：** 本任务现名「接口页面设计师2」，已核实继任「后端设计师2」`01a08fc8-7eba-7d22-81c6-08e41c107d1a`／local，按[通信约定](design-communication.md)建立新通信。之后每项新增或变更设计决策都须用户明确同意，内部技术ACK不能替代；双方每主题最多三轮。每次压缩必须保留通信机制、TODO、合作归属、轮次、待回复、用户决定原文及下一步，恢复先读双方新日志。下方“未核实后端”和历史持续授权是此前快照，不覆盖本条。F02已采用三段顺序、显式配置开关、摘要后确认；用户另要求主界面入口，未批准整页全部恢复或新接口。

**继任进展（2026-09-11）：** 页面已由「接口页面设计师2」`01a08fc7-ff7a-7590-a496-c0bd99e9e26d`（local）接手；[新日志](design-communication-page-2026-09-11.md)记录完整原文恢复、归属及最后收发。继任后端已核实，联系状态见上方现行规则及通信约定，不唤醒前任。当前展示[F02配置编辑原型](../../../../prototypes/repomesh-project-settings-prototype.html)，详见[项目配置专题F02](../../../../current/project-configuration-design.md)；上述三项UI已采用，F02新技术核对仍0／3，不标整行完成，既有契约与采用范围保持。

更新：2026-09-11。工作区：`D:\Project4work\Repomesh_Go_ver`。用户要求归档旧交接并交给下一位页面接口设计师；本文为重新整理的现行交接，不延续旧交接的逐轮追加结构。[旧版归档](../../../2026-09-11-page-api-handoff/HANDOFF-PAGE-API-DESIGN.md)及[归档校验记录](../../../2026-09-11-page-api-handoff/archive-manifest.json)保留历史；通信原文与原型不移动。
...
| --- | --- | --- | --- | --- |
| F01 | 登录、账号关联与App授权引导 | 登录入口和提示已采用；真实流程未设计完整。 | 登录等待/拒绝/过期/返回原位置；账号读取资格与App安装工作能力分开；无安装权限的受邀仓库引导、核查未知。 | 认证/账号关联/安装及完整仓库发现协议；[项目配置](../../../../current/project-configuration-design.md)。 |
| F02 | 已保存项目的配置编辑表单 | 已有契约保持；新增编辑原型r1及专题F02，三段顺序、显式配置开关、摘要后确认已采用，主界面入口按用户要求补齐；新技术核对0／3，不标完成。 | 名称/用途；保留已有范围、明确添加仓库；模型/执行引用；旧修订冲突、原更新结果未知、失权仓仍可修配置。 | [首批浏览器契约](../../../../current/first-batch-browser-api-contract.md)§5/7；[项目配置](../../../../current/project-configuration-design.md)。 |
| F03 | 仓库选择器与授权异常 | 创建两步选择已采用；F03内嵌选择器r1三项页面安排已采用，只模拟partial集合、跨搜索选择与加载更多。 | 完整发现/搜索/分页、选择跨页保留、读权变化清理敏感名称；未知不能当无权或悄悄删仓/扩仓。 | 首批候选接口与尚缺完整发现协议；项目配置J1—J3。 |
| F04 | 模型“用于项目”的完整预览与恢复表单 | 供应商/模型字段已采用；F04弹窗r1三项页面安排已采用，见模型应用专题，专用协议仍未完成。 | 选择有资格项目、明确模型/供应商/Key固定快照、项目修订、保留执行原有效版本；预览变动、冲突、未知恢复和原操作查询。 | **专用应用协议未完成**，现行完整configuration PATCH不能保证仅换模型且精确保留execution版本；[模型设置§3](../../../../current/model-connection-settings-design.md)。 |
| F05 | 密钥管理及供应商异常流程 | 只读密钥状态、演示替换和正常编辑已展示；真实生命周期未设计。 | 真实只写保存、替换/失效影响、测试失败原因/未知、历史观察；禁用/删除/共享/默认切换若纳入产品需先设计范围、再展示，不能照参考图补假按钮。 | Key保留/撤销、权限、费用/出站、逐模型测试及幂等恢复协议；[模型设置](../../../../current/model-connection-settings-design.md)。 |
...
| F07 | 项目/会话/Issue列表筛选与导航状态 | 列表、路由、分页技术基础已有；完整页面状态未走查。 | 筛选/搜索/分页、空/加载/错误/失权、深链接及刷新、旧请求晚到、订阅断流后回读；不将Issue SSE当全站列表订阅。 | 首批契约、[页面专题](../../../../current/conversation-issue-separation-design.md)、创建契约§5.1。 |
| F08 | Issue创建弹窗的完整异常/恢复 | 弹窗、会话选择与提交反馈已采用。 | 已有会话搜索和失效竞态；多提交/跨刷新恢复列表；同键冲突、结果未知、404保留未知、410不复活、失权清理。 | [创建契约](../../../../current/issue-page-create-api-contract.md)，不重新定义第二套提交协议。 |
| F09 | Issue详情概览 | 新增首批最小概览r2待评审，沿历史主题第二轮，未获明确UI采用。 | 与用户确认信息顺序/入口；空/加载/受限、创建成功但准备失败、业务会话与实时房间不同状态。 | 详情原型；首批对象关系/列表/详情契约与运行协议缺口。 |
| F10 | 首次会话、消息发送与澄清答复 | 最小文本与澄清五端点已完成技术设计；消息控件未采用。 | 首次会话建立/第一条消息；草稿、单问题答复、并发答案冲突、改写目标、引用移除；保存/待投递/接收/处理分层与刷新恢复。 | [消息五端点契约](../../../../current/conversation-message-clarification-api-contract.md)、[消息目标专题](../../../../current/conversation-message-target-design.md)；完整B05仍缺。 |
...
| F13 | DAG详情、版本与执行操作面板 | 只读图布局已采用；动态状态/操作表单未完成。 | 计划版本/Loop/Attempt/跨仓采纳、待应用/部分应用；何时允许取消/重试及对应影响；不擅自加入直接编辑图。 | [Graph/Loop](../../../../current/graph-loop-design.md)、ADR-0014/0015具体采用章节、后端B07。 |
...
| F15 | 可选仓库分析表单与结果面板 | 可选辅助方向已采用；完整原型未完成。 | 分析按钮、排队/分析/部分结果/失败/恢复、证据和建议、明确应用、不清手动选择；输入变化旧结果失效，失败可手动建项。 | [仓库分析](../../../../current/issue-creation-repository-analysis.md)、[ADR-0020](../../../../adr/0020-python-repository-analysis-plugin.md)；浏览器字段页面负责，Python Schema后端负责。 |
...
| --- | --- | --- | --- |
| [Issue创建契约](../../../../current/issue-page-create-api-contract.md) | v1；RM-PAGE-01 r1等补充已核 | 双入口创建、关联会话、原操作幂等/恢复、Issue SSE。 | 实际创建服务、数据库或运行接入。 |
| [首批浏览器契约](../../../../current/first-batch-browser-api-contract.md) | RM-API-01 r3，3／3轮已互核 | 12个项目/列表/配置引用相关端点及原创建/更新恢复。 | 完整认证、所有参与仓库发现、Key管理及专用模型应用。 |
| [最小消息与澄清契约](../../../../current/conversation-message-clarification-api-contract.md) | RM-UI-MESSAGE-TARGET r3，3／3轮已互核 | 已有会话五端点、文本保存、唯一答案与固定回执、分页、原键恢复、澄清快照。 | 首次会话全链路、多源拆分、更正、真实传输/Manager执行。 |
| [模型设置专题](../../../../current/model-connection-settings-design.md) | RM-UI-MODEL-SETTINGS r3，3／3轮已互核 | UI目标、锁定AgentTeams字段映射与接口依赖划分。 | REST/密钥存储/逐模型测试/原子应用Schema。 |
| [后端首批持久化](../../../../current/backend-first-batch-persistence.md)及[消息内部设计](../../../../current/backend-message-clarification-design.md) | 分别与首批契约和消息r3对应 | 实体/事务/可信控制的文档设计。 | DB、队列、凭据适配或故障用例已经运行。 |
...
锁定源码：`third_party/AgentTeams`，提交`517caff9280242a00a4d4c06365352b9e41659c6`，见[来源记录](../../../../../third_party/agentteams-source.json)。本轮静态读取安装器、controller配置/initializer、known-models、OpenClaw模板和启动脚本；详细路径/映射见模型设置§1.1及[后端专项§7.8](../../../../current/draft-conversation-backend-design.md)。
...
产品实现按最近工程交接仍为骨架：Web存活检查不代表业务Ready，业务API、协调及受限主机执行未完成；本设计任务未改产品代码、未重跑工程测试。若新会话发现产品有后续实现，按新提交与验证证据更新，不机械沿用此快照。历史上游实验查[证据索引](../../../../current/scaffold-agentteams-evidence.md)，不自动复跑。
